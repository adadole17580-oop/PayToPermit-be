...
feeRoutes.put('/:id', async (c) => {
  try {
    ...
    // Update fee
    if (name) fees[feeIndex].name = name.trim();
    if (amount) fees[feeIndex].amount = parseFloat(amount);
    if (description !== undefined) fees[feeIndex].description = description;
    if (category) fees[feeIndex].category = category.trim();
    if (academic_year) fees[feeIndex].academic_year = academic_year.trim();
    if (required !== undefined) fees[feeIndex].required = required;
