...
export class PaymentController {
    ...
    static async uploadProofOfPayment(c) {
        try {
            ...
            const notificationResults = await Promise.allSettled(adminIds.map((adminId) => NotificationModel.create({
                user_id: adminId,
                role: 'admin',
    ...
    static async approveSubmission(c) {
        try {
    ...
    static async rejectSubmission(c) {
        try {
            const submissionId = parseInt(c.req.param('id') || '0');
            const { adminId } = await c.req.json();
            if (!submissionId) {
    ...
    static async getAllSubmissions(c) {
        try {
    ...
    static async getMonthlyStats(c) {
        try {
    ...
    static async getSubmissionById(c) {
        try {
            const submissionId = parseInt(c.req.param('id') || '0');
            if (!submissionId) {
                return c.json({ success: false, message: 'Submission ID is required' }, 400);
